...
static void chkpqcopydataerr(PGconn *, int);
SEXP 
RS_PostgreSQL_CopyIn(SEXP conHandle, SEXP filename)
{
...
static Rboolean isna(SEXP x, int indx)
{
    ...
    switch(TYPEOF(x)) {
    case LGLSXP:
        result = LOGICAL(x)[indx] == NA_LOGICAL;
        break;
    case INTSXP:
        result = INTEGER(x)[indx] == NA_INTEGER;
        break;
    case REALSXP:
        result = ISNAN(REAL(x)[indx]);
        break;
    case STRSXP:
        result = STRING_ELT(x, indx) == NA_STRING;
        break;
    case CPLXSXP:
        rc = COMPLEX(x)[indx];
        result = ISNAN(rc.r) || ISNAN(rc.i);
        break;
    default:
        break;
    }
...
SEXP
RS_PostgreSQL_CopyInDataframe(SEXP conHandle, SEXP x, SEXP nrow, SEXP ncol)
{
    S_EVALUATOR RS_DBI_connection * con;
    int nr, nc, i, j;
