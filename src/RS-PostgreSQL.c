...
SEXP 
RS_PostgreSQL_init(SEXP config_params, SEXP reload)
{
    S_EVALUATOR
...
SEXP 
RS_PostgreSQL_closeManager(SEXP mgrHandle)
{
 ...
 */
SEXP 
RS_PostgreSQL_cloneConnection(SEXP conHandle)
{
    S_EVALUATOR SEXP mgrHandle;
    RS_DBI_connection *con;
    RS_PostgreSQL_conParams *conParams;
    SEXP con_params;

    /* get connection params used to open existing connection */
    con = (RS_DBI_connection *) RS_DBI_getConnection(conHandle);
...
SEXP 
RS_PostgreSQL_newConnection(SEXP mgrHandle, SEXP con_params)
{
...
SEXP 
RS_PostgreSQL_closeConnection(SEXP conHandle)
{
    S_EVALUATOR RS_DBI_connection * con;
    PGconn *my_connection;
    SEXP status;
     ...
     */
    if (con->conParams) {
        RS_PostgreSQL_freeConParams((RS_PostgreSQL_conParams *) con->conParams);
        con->conParams = NULL;
    }
...
SEXP 
RS_PostgreSQL_exec(SEXP conHandle, SEXP statement)
{
    S_EVALUATOR RS_DBI_connection * con;
    SEXP rsHandle;
    ...
    if (my_result == NULL) {
        ...
        len = strlen(omsg);
        free(dyn_statement);
        errMsg = (char*)malloc(len + 80); /* 80 should be larger than the length of "could not ..."*/
    ...
    if (strcmp(PQresultErrorMessage(my_result), "") != 0) {
        ...
        omsg = PQerrorMessage(my_connection);
        len = strlen(omsg);
        errResultMsg = (char*)malloc(len + 80); /* 80 should be larger than the length of "could not ..."*/
...
RS_DBI_fields *
RS_PostgreSQL_createDataMappings(SEXP rsHandle)
{
    PGresult *my_result;
...
RS_PostgreSQL_fetch(SEXP rsHandle, SEXP max_rec)
{
    S_EVALUATOR RS_DBI_manager * mgr;
    RS_DBI_resultSet *result;
    RS_DBI_fields *flds;
 ...
 */
SEXP 
RS_PostgreSQL_getException(SEXP conHandle)
{
...
SEXP 
RS_PostgreSQL_closeResultSet(SEXP resHandle)
{
...
SEXP 
RS_PostgreSQL_managerInfo(SEXP mgrHandle)
{
    S_EVALUATOR RS_DBI_manager * mgr;
    SEXP output;
...
SEXP 
RS_PostgreSQL_connectionInfo(SEXP conHandle)
{
...
SEXP 
RS_PostgreSQL_resultSetInfo(SEXP rsHandle)
{
    S_EVALUATOR RS_DBI_resultSet * result;
    SEXP output, flds;
    Sint n = 6;
...
SEXP 
RS_PostgreSQL_typeNames(SEXP type)
{
    SEXP typeNames;
    Sint n, *typeCodes;
    int i;
...
SEXP expand_list(SEXP old, Sint new_len);
void add_group(SEXP group_names, SEXP data, Stype * fld_Sclass, Sint group, Sint ngroup, Sint i);
unsigned int check_groupEvents(SEXP data, Stype fld_Sclass[], Sint row, Sint col);
...
SEXP 
RS_DBI_invokeEndGroup(SEXP callObj, SEXP data, const char *group_name, SEXP rho)
{
    S_EVALUATOR SEXP s_x, s_group_name, val;
...
SEXP                      /* output is a named list */
RS_PostgreSQL_dbApply(SEXP rsHandle,      /* resultset handle */
...
unsigned int
check_groupEvents(SEXP data, Stype fld_Sclass[], Sint irow, Sint jcol)
{
    if (irow == 0) {              /* Begin */
...
void
add_group(SEXP group_names, SEXP data, Stype * fld_Sclass, Sint group_field, Sint ngroup, Sint i)
{
    char buff[1024];
