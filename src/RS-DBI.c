...
SEXP 
RS_DBI_allocConnection(SEXP mgrHandle, Sint max_res)
{
    RS_DBI_manager *mgr;
...
void
RS_DBI_allocOutput(SEXP output, RS_DBI_fields * flds, Sint num_rec, Sint expand)
{
    SEXP names, s_tmp;
    Sint j;
    int num_fields;
...
void
RS_DBI_setException(SEXP handle, DBI_EXCEPTION exceptionType, int errorNum, const char *errorMsg)
{
    HANDLE_TYPE handleType;

    handleType = (HANDLE_TYPE) GET_LENGTH(handle);
...
{
    S_EVALUATOR SEXP output, output_names, obj = S_NULL_ENTRY;
    Sint num_elem;
    int j;
