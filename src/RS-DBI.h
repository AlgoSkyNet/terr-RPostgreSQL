...
#define RES_ID(handle) INT_EL((handle),2)
     ...
     */
    SEXP RS_DBI_allocManager(const char *drvName, Sint max_con, Sint fetch_default_rec, Sint force_realloc);
    void RS_DBI_freeManager(SEXP mgrHandle);
    RS_DBI_manager *RS_DBI_getManager(SEXP handle);
    SEXP RS_DBI_asMgrHandle(Sint pid);
    SEXP RS_DBI_managerInfo(SEXP mgrHandle);

    /* dbConnection */
    SEXP RS_DBI_allocConnection(SEXP mgrHandle, Sint max_res);
    void RS_DBI_freeConnection(SEXP conHandle);
    RS_DBI_connection *RS_DBI_getConnection(SEXP handle);
    SEXP RS_DBI_asConHandle(Sint mgrId, Sint conId);
    ...
    SEXP RS_DBI_asResHandle(Sint pid, Sint conId, Sint resId);
     ...
     */
    void RS_DBI_allocOutput(SEXP output, RS_DBI_fields * flds, Sint num_rec, Sint expand);
    ...
    void RS_DBI_setException(SEXP handle, DBI_EXCEPTION exceptionType, int errorNum, const char *errorMsg);
    ...
    SEXP RS_DBI_createNamedList(char **names, Stype * types, Sint * lengths, Sint n);
